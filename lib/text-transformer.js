...
function transformText(text, filePath, options = {}) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inImportsSection) {
      // Empty lines and comments keep us in imports section but don't need transformation
      if (trimmedLine === "" || trimmedLine.startsWith("//") || trimmedLine.startsWith("/*") || trimmedLine.startsWith("*")) {
        transformedLines.push(line);
        continue;
      }
      
      // Import/require statements are part of imports section and need transformation
      if (
