  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inImportsSection) {
      ...
      if (
        ...
        const importMatch = line.match(/^(\s*)import\s+((?:type\s+)?(?:\*\s+as\s+\w+|\{[^}]*\}|\w+)?)\s+from\s+["']([^"']+)["'](;?)$/);
        if (importMatch) {
          const [, leadingWhitespace, importClause, importPath, semicolon] = importMatch;
        ...
        if (requireMatch) {
          const [, leadingWhitespace, varName, importPath, semicolon] = requireMatch;
